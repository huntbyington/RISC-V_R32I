...
class riscof_riscv_top(pluginTemplate):
    __model__ = "riscv_top"
    __version__ = "0.1.0"
    ...
    def runTests(self, testList):
        for testname in testList:
            ...
            # Split the ELF into two flat binary images matched to the DUT's
            # two separate physical memories, then hand-convert each to a
            # $readmemh-compatible word-hex file ourselves.
            #
            # We deliberately do NOT use `objcopy -O verilog
            # --verilog-data-width=4`: that option has long-standing,
            # still-open binutils bugs where the packed word is emitted in
            # the wrong endianness regardless of target (binutils PR 25202),
            # which would silently corrupt every multi-byte instruction/word
            # loaded into the DUT. `-O binary` + our own little-endian
            # packer avoids that entirely.
            ...
            # Pull the addresses the testbench needs to know out of the ELF
            # symbol table, rather than assuming fixed offsets (the linker
            # is free to place tohost/begin_signature/end_signature wherever
            # they fit given each test's own code/data size).
            ...
            # NOTE: riscv_top.sv and friends use `include "src/definitions.vh"`
            # (relative to the *repo root*, not to src/ itself) -- so the
            # include path passed to iverilog must be REPO_ROOT, not SRC_DIR.
            compile_sim = (
